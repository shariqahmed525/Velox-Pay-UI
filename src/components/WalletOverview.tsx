...
export const WalletOverview: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          <h2 className="text-3xl font-bold text-light-text dark:text-dark-text font-display">Portfolio Overview</h2>
      ...
      <motion.div
        ...
        className="bg-gradient-to-br from-primary to-primary-dark rounded-2xl p-8 shadow-glow relative overflow-hidden"
      >
        <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full blur-3xl" />
        <div className="relative">
          <p className="text-white/80 text-sm uppercase tracking-wider">Total Portfolio Value</p>
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          ...
          <div className="flex items-center space-x-2 mt-3">
            ...
            <span className="text-white text-sm">+4.2% this month</span>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {currencies.map((currency, index) => (
          <motion.div
            ...
            className="bg-light-surface/50 dark:bg-dark-surface/50 backdrop-blur-sm border border-light-border dark:border-dark-border rounded-xl p-6 hover:border-primary/30 transition-all hover:shadow-glow group duration-300"
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                <span className="text-2xl">{currency.flag}</span>
                <div>
                  <h3 className="font-bold text-light-text dark:text-dark-text font-display">{currency.code}</h3>
                  <p className="text-xs text-light-text-secondary dark:text-dark-text-secondary">Balance</p>
                </div>
              </div>
              <div className={`flex items-center space-x-1 ${currency.change >= 0 ? 'text-success' : 'text-error'}`}>
                {currency.change >= 0 ? (
            ...
            <div className="space-y-2">
              <motion.p
                ...
                className="text-2xl font-bold text-light-text dark:text-dark-text font-display"
              >
