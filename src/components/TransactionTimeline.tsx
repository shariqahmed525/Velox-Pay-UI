...
export const TransactionTimeline: React.FC = () => {
  return (
    <div className="space-y-6">
      ...
      >
        <h2 className="text-3xl font-bold text-light-text dark:text-dark-text font-display">Recent Activity</h2>
      ...
      <div className="bg-light-surface/50 dark:bg-dark-surface/50 backdrop-blur-sm border border-light-border dark:border-dark-border rounded-2xl p-6 shadow-glass transition-colors duration-300">
        <div className="space-y-4">
          {transactions.map((transaction, index) => (
            ...
            >
              {/* Transaction Icon */}
              <div className={`p-3 rounded-full ${transaction.type === 'sent' ? 'bg-error/20' : 'bg-success/20'}`}>
                {transaction.type === 'sent' ? (
                  <ArrowUpRight className={`w-5 h-5 ${transaction.type === 'sent' ? 'text-error' : 'text-success'}`} />
                ) : (
                  <transaction.icon className="w-5 h-5 text-success" />
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center space-x-2 mb-1">
                  <p className="font-medium text-light-text dark:text-dark-text font-display truncate">{transaction.recipient}</p>
              ...
              <div className="text-right">
                <motion.p
                  ...
                  className={`font-bold font-display text-lg ${
                    transaction.amount > 0 ? 'text-success' : 'text-light-text dark:text-dark-text'
                  }`}
                >
              ...
              <motion.div
                ...
                className="absolute bottom-0 left-0 h-px bg-primary/30"
              />
            </motion.div>
          ))}
        </div>
